<template>
  <main>
    <section class="hero_in prjcts">
      <div class="wrapper">
        <div class="container">
          <h1 class="fadeInUp"><span></span>Projects in {{ location }}</h1>
        </div>
      </div>
    </section>
    <!--/hero_in-->
    ...
    <div class="container margin_60_35">
      <div class="row" v-if="viewMode === 'isotope'">
        ...
        <div class="col-lg-9" id="list_sidebar">
          <div class="isotope-wrapper">
            <b-card
              no-body
              v-if="!response.loader && !response.projects.length"
            >
              <b-card-body>
                <b-row>
            ...
            <div v-if="!response.loader">
              <ProjectIsotope
                v-for="item in response.projects"
                :key="item.name"
                :item="item"
              />
            </div>
      ...
      <p
        ...
        v-if="response.count && response.projects.length < response.count"
      >
        <a
          ...
          >{{
            response.loader
              ? "Loading ..."
              : `Load more - ${response.projects.length}/${response.count}`
          }}</a
        >
      </p>
    </div>
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      await this.$apollo
        .query({
          query: require("@/graphQl/queries/project/list.gql"),
          // variables: this.variables,
          fetchPolicy: "network-only",
        })
        .then(({ data }) => {
          const { projects = [], projects_aggregate } = data;
          this.response.projects = [
            ...this.response.projects,
            ...mapProjectData(projects),
          ];
          this.response.count = projects_aggregate.aggregate.count || 0;
          setTimeout(() => {
    ...
    listenEvents(existingLoc) {
      EventBus.$on("EVENT:setLocation", (loc) => {
        ...
        if (
          loc &&
          loc !== existingLoc &&
          this.$route.name.match(/-list/g)
        ) {
          const queryParams = { ...this.$route.query, city: loc };
          const routeConfig = Object.keys(queryParams).length
            ? { path: "/projects-list", query: queryParams }
            : { path: "/projects-list" };
