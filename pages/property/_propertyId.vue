          ...
          <div class="row">
            <div class="col-lg-8">
              <section id="overview">
                <h2>Overview</h2>
                <p>
                  {{ property.description || marketing_description }}
                </p>
...
export default {
  ...
  methods: {
    ...
    async fetchData(pID) {
      await this.$apollo
        .query({
          query: require("@/graphQl/queries/property-detail.gql"),
          variables: {
            id: pID,
          },
        })
        .then(({ data }) => {
          const { property_by_pk } = data;
          const mappedProperty = mapPropertyData([property_by_pk])[0];
          if (!mappedProperty) {
            return this.redirecToHome();
          }
  ...
  mounted() {
    ...
    this.fetchData(this.$route.params.propertyId);
