...
<script>
import { mapProjectData } from "../../helpers/api";
export default {
  data() {
    return {
      project: null,
  ...
  methods: {
    ...
    async fetchData(pID) {
      await this.$apollo
        .query({
          query: require("@/graphQl/queries/project/view.gql"),
          variables: {
            id: pID,
          },
        })
        .then(({ data }) => {
          const { projects_by_pk } = data;
          const mappedProject = mapProjectData([projects_by_pk])[0];
          if (!mappedProject) {
